...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(n) if n - n.toInt == 0 => n.toInt }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction { case Json.Arr(arr) if arr.forall(e => decoder.decode(e).isDefined) =>
      val empty = List[A]()
      arr.foldLeft(List[A]())((l, e) => l :+ decoder.decode(e).get)
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction { case Json.Obj(fields) =>
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.fromPartialFunction { case Json.Obj(fields) =>
      (for {
        jName <- fields.get("name")
        name <-  jName.decodeAs[String]
        jAge <- fields.get("age")
        age <-  jAge.decodeAs[Int]
      } yield Person(name, age)).orNull
    }
